...
namespace zs {
  ...
  constexpr void iter_neighbors(const BvhView &bvh, const BV &bv, F &&f) {
    ...
    if (auto nl = bvh.numLeaves(); nl <= 128) {
      for (index_t i = 0; i != nl; ++i) {
  ...
  template <typename Policy> void LBvh<dim, lane_width, Index, Value, Allocator>::build(
    ...
    if (numLeaves <= 128) {  // edge cases where not enough primitives to form a tree
    ...
    policy(zip(range(numLeaves), splits), ComputeSplitMetric{execTag, numLeaves, sortedMcs, totalBits});
    ...
    policy(zip(range(numLeaves - 1), trunkDst, trunkR),
