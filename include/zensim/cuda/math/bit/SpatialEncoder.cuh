...
namespace zs {
  ...
  struct coord_to_offset {
    ...
    __host__ coord_to_offset(CoordType dxinv, std::array<MaskType, Dim> masks)
        : _dxInv(std::move(dxinv)) {
    ...
    __forceinline__ __host__ __device__ auto operator()(int dimNo, In&& in) -> MaskType {
      static_assert(std::is_convertible<In, CoordType>::value,
                    "coord_to_offset: coordinate type not convertible.");
#if 0
      ...
      return bit_spread_64(_masks[dimNo], static_cast<IndexType>(
                                              std::forward<In>(in) * static_cast<CoordType>(_dxInv)
                                              + static_cast<CoordType>(0.5))
                                              - 1);
    }
  ...
  template <typename MaskType, typename IndexType, int Dim = 3> struct offset_to_index {
  ...
  template <typename SPGTraits> struct compute_neighbor_key {
    static constexpr int dim = SPGTraits::dimension;
    template <int neighborid, typename SPGTs = SPGTraits>
    __forceinline__ __host__ __device__ std::enable_if_t<SPGTs::dimension == 2, uint64_t>
    operator()(uint64_t key) {
      constexpr uint64_t neighborOffset
          = SPGTraits::neighbor_block_offset(neighborid & 2 ? 1 : 0, neighborid & 1);
      // printf("%d-th neighbor %llx\n", neighborid, neighborOffset);
      uint64_t x_result
          = ((key | SPGTraits::MXADD_Xmask) + (neighborOffset & ~SPGTraits::MXADD_Xmask))
            & ~SPGTraits::MXADD_Xmask;
      uint64_t y_result
          = ((key | SPGTraits::MXADD_Ymask) + (neighborOffset & ~SPGTraits::MXADD_Ymask))
            & ~SPGTraits::MXADD_Ymask;
      uint64_t w_result
          = ((key | SPGTraits::MXADD_Wmask) + (neighborOffset & ~SPGTraits::MXADD_Wmask))
            & ~SPGTraits::MXADD_Wmask;
      uint64_t result = x_result | y_result | w_result;
      return result >> SPGTraits::page_bits;
    }
    template <int neighborid, typename SPGTs = SPGTraits>
    __forceinline__ __host__ __device__ std::enable_if_t<SPGTs::dimension == 3, uint64_t>
    operator()(uint64_t key) {
      ...
      uint64_t y_result
          = ((key | SPGTraits::MXADD_Ymask) + (neighborOffset & ~SPGTraits::MXADD_Ymask))
            & ~SPGTraits::MXADD_Ymask;
      uint64_t z_result
          = ((key | SPGTraits::MXADD_Zmask) + (neighborOffset & ~SPGTraits::MXADD_Zmask))
            & ~SPGTraits::MXADD_Zmask;
      uint64_t w_result
          = ((key | SPGTraits::MXADD_Wmask) + (neighborOffset & ~SPGTraits::MXADD_Wmask))
            & ~SPGTraits::MXADD_Wmask;
