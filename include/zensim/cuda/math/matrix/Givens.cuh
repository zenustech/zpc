...
namespace zs {
  ...
  namespace math {
    ...
    public:
      ...
      __forceinline__ __host__ __device__ void transposeInPlace() { s = -s; }
      ...
      __forceinline__ __host__ __device__ std::enable_if_t<std::is_same<TT, float>::value, void>
      compute(const TT a, const TT b) {
        TT d = a * a + b * b;
        c = 1;
        s = 0;
        TT sqrtd = sqrtf(d);
      ...
      template <typename TT>
      __forceinline__ __host__ __device__ std::enable_if_t<std::is_same<TT, double>::value, void>
      compute(const TT a, const TT b) {
        TT d = a * a + b * b;
        c = 1;
        s = 0;
      ...
      template <typename TT>
      __forceinline__ __host__ __device__ std::enable_if_t<std::is_same<TT, float>::value, void>
      computeUnconventional(const TT a, const TT b) {
        TT d = a * a + b * b;
        c = 0;
        s = 1;
        TT sqrtd = sqrtf(d);
      ...
      template <int Dim, typename T1> __forceinline__ void vecRotation(T1 A[Dim]) const {
        T1 tau1 = A[rowi];
        T1 tau2 = A[rowk];
      ...
      /**
        Multiply givens must be for same row and column
        **/
      __forceinline__ __host__ __device__ GivensRotation<T> operator*(
          const GivensRotation<T>& A) const {
