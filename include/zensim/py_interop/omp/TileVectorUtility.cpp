...
#define INSTANTIATE_TILE_VECTOR_CAPIS(T, L)                                              \
  ZPC_EXPORT void append_properties##__##omp##_##tv##_##T##_##L(                         \
      zs::OmpExecutionPolicy *ppol, zs::TileVector<T, L, zs::ZSPmrAllocator<false>> *v,  \
      const std::vector<zs::PropertyTag> *tags) {                                        \
    v->append_channels(*ppol, *tags);                                                    \
  }                                                                                      \
  ZPC_EXPORT void append_properties##__##omp##_##tv##_##T##_##L##_##virtual(             \
      zs::OmpExecutionPolicy * ppol, zs::TileVector<T, L, zs::ZSPmrAllocator<true>> * v, \
      const std::vector<zs::PropertyTag> *tags) {                                        \
    v->append_channels(*ppol, *tags);                                                    \
  }
