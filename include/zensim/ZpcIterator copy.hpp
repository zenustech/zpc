...
namespace zs {
  ...
  public:
    ...
    friend constexpr Derived &operator+=(Derived &it,
                                         detail::infer_difference_type_t<Self> offset) {
      it.advance(offset);
      return it;
    }
    template <typename Self = Derived, enable_if_t<detail::has_advance<Self>::value> = 0>
...
#define LEGACY_ITERATOR_OP(OP)                                                        \
  ...
  template <typename Iterator> struct LegacyIterator : Iterator {
    ...
    friend constexpr LegacyIterator &operator+=(LegacyIterator &it, difference_type offset) {
      it.advance(offset);
      return it;
    }
    template <typename Self = base_t, enable_if_t<detail::has_advance<Self>::value> = 0>
    ...
    template <typename Self = base_t, enable_if_t<detail::has_advance<Self>::value> = 0>
    friend constexpr LegacyIterator operator+(difference_type offset, LegacyIterator it) {
      return it += offset;
    }
    template <typename Self = base_t, enable_if_t<detail::has_advance<Self>::value> = 0>
    friend constexpr LegacyIterator operator-(LegacyIterator it, difference_type offset) {
  ...
  template <typename... Ts> constexpr bool all_convertible_to_raiter() {
    return (is_convertible_v<typename iterator_traits<Ts>::iterator_category,
                             random_access_iterator_tag>
            && ...);
  }
...
#if 0
        = conditional_t<all_convertible_to_raiter<Iters...>(),
                        common_type_t<typename iterator_traits<Iters>::difference_type...>, void>;
#endif
  ...
  template <typename... Args> constexpr auto zip(Args &&...args) {
    auto bg = make_iterator<zip_iterator>(std::begin(FWD(args))...);
    auto ed = make_iterator<zip_iterator>(std::end(FWD(args))...);
  ...
  template <typename... Args> constexpr auto enumerate(Args &&...args) {
    auto bg = make_iterator<zip_iterator>(make_iterator<IndexIterator>((sint_t)0),
                                          std::begin(FWD(args))...);
    auto ed = make_iterator<zip_iterator>(make_iterator<IndexIterator>(limits<sint_t>::max()),
                                          std::end(FWD(args))...);
    return detail::iter_range(zs::move(bg), zs::move(ed));
  }
