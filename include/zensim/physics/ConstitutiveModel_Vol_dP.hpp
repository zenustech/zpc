...
namespace zs {
  ...
  constexpr void compute_stress_fixedcorotated(T volume, T mu, T lambda, const vec<T, 9> &F,
                                               vec<T, 9> &PF) {
    T U[9] = {}, S[3] = {}, V[9] = {};
  ...
  template <typename T = float>
  constexpr void compute_stress_vonmisesfixedcorotated(T volume, T mu, T lambda, T yield_stress,
  ...
  template <typename T = float>
  constexpr void compute_stress_nacc(T volume, T mu, T lambda, T bm, T xi, T beta, T Msqr,
...
#pragma unroll 3
        for (int i = 0; i < 3; i++)
          S[i] = math::sqrt(s_hat_trial[i] * B_s_coeff + trace_B_hat_trial_divdim);
        T New_F[9] = {};
        matmul_mat_diag_matT_3D(New_F, U, S, V);
#pragma unroll 9
        for (int i = 0; i < 9; i++) F[i] = New_F[i];
        ...
        if (hardeningOn && p0 > 1e-4 && p_trial < p0 - 1e-4 && p_trial > 1e-4 + p_min) {
          ...
          T q_trial = math::sqrt(3.f / 2.f * s_hat_trial_sqrnorm);
          T direction[2] = {p_center - p_trial, -q_trial};
          T direction_norm = math::sqrt(direction[0] * direction[0] + direction[1] * direction[1]);
          ...
          T l1 = (-B + math::sqrt(B * B - 4 * A * C)) / (2 * A);
          T l2 = (-B - math::sqrt(B * B - 4 * A * C)) / (2 * A);
  ...
  template <typename T = float>
  constexpr void compute_stress_sand(T volume, T mu, T lambda, T cohesion, T beta, T yieldSurface,
                                     bool volCorrection, T &logJp, vec<T, 9> &F, vec<T, 9> &PF) {
    T U[9] = {}, S[3] = {}, V[9] = {};
    math::svd_3d(F[0], F[3], F[6], F[1], F[4], F[7], F[2], F[5], F[8], U[0], U[3], U[6], U[1], U[4],
                 U[7], U[2], U[5], U[8], S[0], S[1], S[2], V[0], V[3], V[6], V[1], V[4], V[7], V[2],
                 V[5], V[8]);
...
#pragma unroll 3
    ...
    T epsilon_hat_norm
        = math::sqrt(epsilon_hat[0] * epsilon_hat[0] + epsilon_hat[1] * epsilon_hat[1]
